...
function testScalaChess() {
    try {
        console.info("START scalachessjs test");
        ...
        res = scalachessjs.Main().getDests(getDests);
        ...
        console.info("END scalachessjs test");
    } catch(err) {
...
function testChessJS() {
    console.info("START testChessJS test");
    console.time('getDests');
    chess = new Chess(getDests.fen);
    // console.info("Moves: " + util.inspect(chess.moves({verbose:true}),{ showHidden: true, depth: null }) );
    console.timeEnd('getDests');

    console.time('threeFoldTest');
    chess = new Chess(threefold.initialFen);
    for(move of threefold.pgnMoves) {
        chess.move(move);
        if(chess.game_over()) {
            // console.log("Game over at move: " + move);
            break;
        }
    }
    console.timeEnd('threeFoldTest');
    ...
    chess = new Chess();
    chess.load_pgn(pgn);
    //console.info("Moves: " + util.inspect(chess.history({verbose:true}),{ showHidden: true, depth: null }) );
    console.timeEnd('pgnRead');
    ...
    //console.info("PGN: " + util.inspect(chess.pgn(),{ showHidden: true, depth: null }) );
    console.timeEnd('pgnDump');

    console.info("END testChessJS test");
}
